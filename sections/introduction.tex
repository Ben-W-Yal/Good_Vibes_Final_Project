...
Understanding modern conflict increasingly means making sense of fast-moving, fragmentary signals: \textbf{social-media posts} (often the first ripple around protests, strikes, or disasters), wire copy, tracker data, and satellite news that rarely arrive in one place. Real-time social reporting can surface genuine eyewitness detail; it can also spread rumor, coordinated inauthentic behavior, and context-stripped clips. Consumers and analysts face the same failure mode: headlines without terrain, timelines without geography, and narratives that age out before context arrives. GeoIntel addresses this gap by coupling those signals---as they appear in our normalized event feeds---with interactive mapping and generative summarization so users relate events to \emph{where} they occur and \emph{what} surrounds them---terrain, urban structure, coastlines, and borders---rather than scanning disconnected feeds.

\subsection{Problem statement and intended users}

The core problem is \textbf{situational overload}: too many sources (including high-volume \textbf{social-derived} and news-indexed events), too little integration, and weak coupling between narrative reporting and geographic reality. GeoIntel targets students, analysts, journalists, and technically minded observers who need a single workspace to explore conflict-related signals, overlay mobility where available, and produce structured written assessments grounded in web evidence.

\subsection{Plain-language system description}

\textbf{Inputs} include configurable OSINT-style event feeds (e.g., GDELT-style document and media intensity~\cite{gdelt}, which indexes global news and \textbf{social-linked} discussion, plus structured conflict datasets such as ACLED~\cite{acled} where enabled), optional live trackers for aircraft, maritime traffic, and satellite propagation from public catalogs~\cite{celestrak}, plus user-selected filters for region and time.
...
Express~\cite{express} API routes in TypeScript orchestrate retrieval, normalization, and prompting. We \textbf{deliberately avoid open-ended agentic loops}: behavior is \emph{deterministic}---section-scoped PDB prompts, fixed \texttt{/api/chat} modes, and server-chosen model parameters---so outputs stay auditable and predictable rather than tool-chasing. Heavy requests stay off the UI thread; optional Python tooling exists only for peripheral bridges (e.g., SpaceMouse) described in Implementation.

\subsection{Stable context versus incoming signals}

Analysts usually keep a slow-changing mental map (theater, terrain, baseline relationships) while processing fast-arriving headlines and tracker updates. GeoIntel mirrors that rhythm in software: persistent globe state, filters, and adapter-backed caches sit alongside refreshed markers, search-grounded brief sections, and streamed per-event text. We do \textbf{not} ship a formal recursive filter or latent-state model in code; the point is UX and orchestration that discourage one-shot narratives detached from the live map and feed state.
