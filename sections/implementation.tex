\section{Implementation}

\subsection{Architecture overview}

GeoIntel follows a classic three-tier layout: a React~\cite{react} single-page client, an Express~\cite{express} HTTP API in TypeScript, and pluggable upstream adapters that normalize heterogeneous feeds into shared schemas before the UI renders them on a CesiumJS globe~\cite{cesium}.

\begin{enumerate}[leftmargin=*]
  \item \textbf{Client (}\texttt{client/}\textbf{):} React and TypeScript; CesiumJS for the 3D map; panels for filters, events, trackers, and the Presidential Daily Brief (PDB).
  \item \textbf{Server (}\texttt{server/}\textbf{):} Express registers REST routes under \texttt{/api/...}; shared adapter logic lives under \texttt{src/adapters/} with validation (e.g., Zod) where applicable.
  \item \textbf{Data plane:} External providers (news/conflict feeds, trackers, satellite TLE sources) are fetched server-side only; credentials stay in environment variables loaded at startup (\texttt{dotenv}).
\end{enumerate}

Figure~\ref{fig:architecture} sketches the main trust boundary: the browser talks only to Express; adapters and Gemini calls run server-side with keys from the host environment.

\begin{figure}[!htbp]
  \centering
  \begin{tikzpicture}[
      scale=0.82,
      transform shape,
      font=\footnotesize,
      box/.style={
        rectangle,
        draw=black!72,
        rounded corners=2.5pt,
        line width=0.55pt,
        align=center,
        inner sep=5pt,
        minimum width=2.65cm,
      },
    ]
    \node[box] (ui) {\textbf{Browser client}\\[-0.15em]{\scriptsize React \textbullet{} TypeScript \textbullet{} Cesium}};
    \node[box, below=1.05cm of ui] (api) {\textbf{Express server}\\[-0.15em]{\scriptsize \texttt{/api/...} routes}};
    \node[box, below left=1.05cm and 0.15cm of api] (adp) {\textbf{Feed adapters}\\[-0.15em]{\scriptsize \texttt{src/adapters/}}};
    \node[box, below right=1.05cm and 0.15cm of api] (gem) {\textbf{Gemini calls}\\[-0.15em]{\scriptsize PDB + \texttt{/api/chat}}};
    \node[box, below=1.15cm of api, yshift=-0.05cm] (ext) {\textbf{Upstream services}\\[-0.15em]{\scriptsize feeds, trackers, Google APIs}};
    \draw[-{Latex[length=2.1mm]}] (ui) -- node[right, font=\scriptsize] {HTTP(S)} (api);
    \draw[-{Latex[length=2.1mm]}] (api) -- (adp);
    \draw[-{Latex[length=2.1mm]}] (api) -- (gem);
    \draw[-{Latex[length=2.1mm]}] (adp) -- (ext);
    \draw[-{Latex[length=2.1mm]}] (gem) -- (ext);
  \end{tikzpicture}
  \caption{Trust boundary and major components (not an autonomous agent loop). Adapters normalize providers; generative routes call Gemini with prompts built from stored context and (for PDB) search tools when enabled.}
  \label{fig:architecture}
\end{figure}
\FloatBarrier

Development uses \texttt{tsx} to run TypeScript directly; production builds bundle the server (\texttt{npm run build}, \texttt{npm start}). The client is developed with Vite and served through the same dev server in typical setups described in the repository README.

\subsection{Globe, terrain, and imagery}

The map uses \textbf{CesiumJS}. Optional \textbf{Cesium ion} terrain and OSM-style building enhancements are toggled via client env vars (e.g., \texttt{VITE\_CESIUM\_ION\_TOKEN}, \texttt{VITE\_CESIUM\_ENABLE\_TERRAIN}, \texttt{VITE\_CESIUM\_ENABLE\_BUILDINGS}). Without a token, the globe still runs with reduced 3D fidelity.

Brief sections can attach \textbf{grounded visuals}: map focus derived from modeled regions plus article imagery extracted from citation pages when embeddable images exist. That pairs narrative output with geographic and visual context.

\subsection{Feeds and trackers}

Each external source has an adapter module under \texttt{src/adapters/}. Examples include GDELT-style document queries~\cite{gdelt}, ACLED where credentials allow~\cite{acled}, AIS streams and AISHub for maritime context, OpenSky and optional supplemental aircraft sources~\cite{opensky}, and CelesTrak-based TLE propagation for satellites~\cite{celestrak}. Responses are cached server-side with TTLs to respect rate limits and stability.

\subsection{Generative AI: Presidential Daily Brief}

The PDB workflow calls Google's \textbf{Gemini} models through the Gemini API~\cite{gemini-api}. When search grounding is enabled, the server sends requests that include the Google Search tool so outputs can cite retrieved URLs.
The client renders citations as links and persists successful section payloads in browser storage for resilience across reloads.

Generation proceeds \textbf{section by section} (one request per thematic slide), which is a deliberate multi-call pipeline rather than a single monolithic completion: it trades extra latency for clearer failure isolation and UI progress feedback.

\textbf{Agentic tooling:} We do not rely on open-ended autonomous agents browsing the web on their own. The closest analogue is grounded retrieval plus constrained JSON-oriented prompting inside fixed API routes---deterministic orchestration from the server, not emergent agent loops.

\subsection{Per-event AI analyst: summary and chat}

Beyond the PDB deck, the client exposes \textbf{event-scoped} generative assistance in the event detail panel (\texttt{EventDetail.tsx}).

When the user selects an event, the UI builds a structured \texttt{eventContext} string from the normalized record: title, description, background, place, coordinates, severity, primary source URL, timestamps, and up to ten related article titles/URLs.

It then calls \texttt{POST /api/chat} with \texttt{mode: "summary"} and a fixed analyst-style prompt requesting four short paragraphs (situation, actors and location, strategic ``so what,'' and a watch-for line). The server streams Gemini tokens back as plain text for progressive display.

The \textbf{follow-up chat} reuses the same route with \texttt{mode: "chat"}, appending user and assistant turns. The server applies a consistent analyst persona and instructions to anchor claims in the supplied context, avoid inventing casualty figures or quotes, and preserve conversational continuity. Temperature and output token limits differ by mode (tighter for chat). This path does not invoke Google Search grounding; it relies on the event payload and related links already ingested by adapters---complementary to the PDB's web-grounded sections.

\subsection{Deterministic generative paths}

Figure~\ref{fig:llm-flow} summarizes the \textbf{fixed} multi-step pattern users trigger: browser requests hit explicit Express handlers; each PDB section is a separate completion with optional Google Search grounding~\cite{gemini-api}; event summary and chat call the same \texttt{/api/chat} route with \texttt{eventContext} but without a planning agent that chooses tools dynamically. Responses stream to the UI. This satisfies the course expectation to describe multi-step LLM flows while stating clearly that we do \emph{not} run open-ended autonomous agents.

\begin{figure}[!htbp]
  \centering
  \begin{tikzpicture}[
      scale=0.88,
      transform shape,
      font=\scriptsize,
      node distance=0.55cm and 0.75cm,
      box/.style={
        rectangle,
        draw=black!72,
        rounded corners=2pt,
        inner sep=4pt,
        align=center,
        minimum width=1.55cm,
      },
    ]
    \node[box] (cli) {\textbf{Browser}};
    \node[box, right=of cli] (api) {\textbf{Express}};
    \node[box, right=of api] (gem) {\textbf{Gemini}};
    \node[box, right=of gem] (ui) {\textbf{UI}\\stream};
    \node[box, above=0.5cm of gem, dashed, inner sep=3pt, font=\tiny] (sea) {Search\\(PDB)};
    \draw[-{Latex[length=1.8mm]}] (cli) -- node[above, font=\tiny] {HTTP} (api);
    \draw[-{Latex[length=1.8mm]}] (api) -- node[above, font=\tiny] {route} (gem);
    \draw[-{Latex[length=1.8mm]}] (gem) -- node[above, font=\tiny] {SSE} (ui);
    \draw[-{Latex[length=1.8mm]}, dashed] (api.north) to[bend left=25] (sea.west);
    \draw[-{Latex[length=1.8mm]}, dashed] (sea.east) to[bend left=25] (gem.north);
  \end{tikzpicture}
  \caption{Simplified generative path: PDB may attach the Google Search tool per section; event chat uses packaged context only. Arrows are intentional server orbits, not emergent agent planning.}
  \label{fig:llm-flow}
\end{figure}
\FloatBarrier

\subsection{PDF export}

Brief PDF export is handled \textbf{in the browser}: the UI captures rendered slide content and emits a PDF (client-side libraries), avoiding server-side storage of generated documents.

\subsection{Optional SpaceMouse bridge}

For 3D navigation hardware, an optional \textbf{Python} bridge (\texttt{script/spacemouse\_bridge.py}) can expose SpaceMouse motion over a local WebSocket (\texttt{127.0.0.1:8765}). \texttt{npm run dev} attempts to start this bridge when the port is free; \texttt{npm run dev:no-spacemouse} skips it.

\subsection{Configuration and secrets}

Secrets (Gemini keys, tracker keys, ion tokens, etc.) live only in \texttt{.env} or equivalent host environment.
They are never committed to git, never pasted into this PDF, and \textbf{never legible in screenshots} prepared for class submission.

The repository ships \texttt{.env.example} as a template; operators copy it to \texttt{.env} locally.

\subsection{Reproducibility}

This write-up mirrors the deliverable expectations for the course final project (written report + code repository with runnable instructions)~\cite{course-req}.

\textbf{Repository:} \url{https://github.com/Ben-W-Yal/Good_Vibes_Final_Project}

\textbf{Steps to run locally:}
\begin{enumerate}[leftmargin=*]
  \item Clone the repo and install dependencies: \texttt{npm install}.
  \item Copy \texttt{.env.example} to \texttt{.env} and add at minimum a valid Gemini API key plus optional provider keys required for trackers or enhanced terrain.
  \item Start development: \texttt{npm run dev} (or \texttt{npm run dev:no-spacemouse} without the SpaceMouse bridge).
  \item Open the served URL (default port shown in server logs, commonly \texttt{localhost:3000} in development).
\end{enumerate}
